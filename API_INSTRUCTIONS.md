# LLM-Aided OCR API - Complete Instructions
...
echo "Submitting PDF for processing..."
...
while true; do
  ...
  echo "Status: $STATUS ($((${PROGRESS} * 100))%)"
...
curl -H "Authorization: Bearer $TOKEN" \
  "$API_URL/download/$JOB_ID/document_llm_corrected.md" \
  -o corrected_output.md

curl -H "Authorization: Bearer $TOKEN" \
  "$API_URL/download/$JOB_ID/document__raw_ocr_output.txt" \
  -o raw_ocr.txt
...
401 Unauthorized
```
**Solution:** Verify the `API_SECRET_TOKEN` in your `.env` file matches the token in your requests.
